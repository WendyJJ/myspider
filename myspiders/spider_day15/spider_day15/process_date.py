...
def main():
    ...
    #无限循环
    while True:
        source,data = rediscli.blpop(["wuyi:items"]) #从redis里提取数据
        item = json.loads(data.decode('utf-8')) #把json转字典
        try:
            #使用cursor()方法获取操作游标
            cur = mysqlcli.cursor()
            #使用execute方法执行SQL insert语句
            sql = 'insert into wuyi_job(url,pname,location,company,smoney,emoney,ptype,person_num,tags,welfare,syear,eyear,time_pub,degree,desc_job,crawl_time,webname) ' \
